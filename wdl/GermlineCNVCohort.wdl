...
workflow CNVGermlineCohortWorkflow {
    ...
    if (select_first([filter_intervals, true])) {
      call CNVTasks.FilterIntervals as FilterIntervalsForPloidy {
        input:
            intervals = preprocessed_intervals,
            exclude_intervals = exclude_intervals_for_filter_intervals_ploidy,
      ...
      call CNVTasks.FilterIntervals as FilterIntervalsForCNV {
        input:
            intervals = preprocessed_intervals,
            exclude_intervals = exclude_intervals_for_filter_intervals_cnv,
            annotated_intervals = AnnotateIntervals.annotated_intervals,
            read_count_files = counts,
            minimum_gc_content = minimum_gc_content,
            maximum_gc_content = maximum_gc_content,
    ...
    File intervals_for_ploidy = select_first([FilterIntervalsForPloidy.filtered_intervals, preprocessed_intervals])
    File intervals_for_cnv = select_first([FilterIntervalsForCNV.filtered_intervals, preprocessed_intervals])

    call DetermineGermlineContigPloidyCohortMode {
        input:
            cohort_entity_id = cohort_entity_id,
            intervals = intervals_for_ploidy,
            read_count_files = counts,
    ...
    call CNVTasks.ScatterIntervals {
        input:
            interval_list = intervals_for_cnv,
    ...
    scatter (scatter_index in range(length(ScatterIntervals.scattered_interval_lists))) {
        call GermlineCNVCallerCohortMode {
            input:
                ...
                runtime_attr_override = runtime_attr_cohort
        }
    }
    ...
    output {
        ...
        File? filtered_intervals_ploidy = FilterIntervalsForPloidy.filtered_intervals
        File? filtered_intervals_cnv = FilterIntervalsForCNV.filtered_intervals
        ...
        Array[Array[File]] gcnv_calls_tars = call_tars_sample_by_shard
...
task GermlineCNVCallerCohortMode {
    ...
    command <<<
        ...
        while [ $CURRENT_SAMPLE -lt $NUM_SAMPLES ]; do
            ...
            tar c -C ~{output_dir_}/~{cohort_entity_id}-calls/SAMPLE_$CURRENT_SAMPLE . \
                | gzip -1 \
                > ~{cohort_entity_id}-gcnv-calls-shard-~{scatter_index}-sample-$CURRENT_SAMPLE_WITH_LEADING_ZEROS.tar.gz

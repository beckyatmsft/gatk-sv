...
workflow GATKSVPipelineBatch {
  ...
  Boolean collect_pesr_ = !(defined(pe_files_input) && defined(sr_files_input) && defined(ld_files_input))
  ...
  Array[File] ld_files_ = if collect_pesr_ then select_all(select_first([GatherSampleEvidenceBatch.pesr_ld])) else select_first([ld_files_input])

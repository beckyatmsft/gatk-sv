...
task FilterVcf {
  ...
  runtime {
    ...
    cpu: select_first([runtime_override.cpu_cores, runtime_default.cpu_cores])
    docker: sv_base_mini_docker
    disk: "local-disk" + select_first([runtime_override.disk_gb, runtime_default.disk_gb]) + " " + disk_type
    preemptible: true
    maxRetries: 3
  }
...
task RenameVariantIds {
  ...
  runtime {
    ...
    cpu: select_first([runtime_override.cpu_cores, runtime_default.cpu_cores])
    docker: sv_base_mini_docker
    disk: "~select_first([runtime_override.disk_gb, runtime_default.disk_gb]) ~{disk_type}"
    preemptible: true
    maxRetries: 3
  }
